...
use make targets for everything.
create github action for test and linter and building 
build for all possible platforms in github actions but build only for the current machine target by default
...
- **Zero Allocations**: Critical paths (marked with `// performance-critical`) should avoid heap allocations.
- **Compatibility**: Maintain Go 1.21+ compatibility. Avoid external dependencies unless necessary.
- **Documentation**: Every exported function/type MUST have a comment starting with the name.
